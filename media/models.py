...
class UpYunStorage(Storage):
    ...
    up = upyun.UpYun(
        BUCKET_NAME, USERNAME, PASSWORD, timeout=30, endpoint=upyun.ED_AUTO)

    def _save(self, name, content):
        full_url = self.BASE_URL + name
        try:
            self.up.put(name, content.read(), checksum=False)
...
class UpyunMedia(models.Model):
    name = models.CharField(
        max_length=40, blank=True, verbose_name=u'file_name')
    url = models.FileField(
        upload_to='media', storage=UpYunStorage(), verbose_name=u'URL')
